...
def dijkstra(source, targets, grid):  # stop after finding all targets
    path = {(x, y): None for x, y in
            product(range(grid.grid_columns + 1), range(grid.grid_rows + 1))}  # check if +1 relevant
...
def goal_test_mst(state_node) -> bool:
    ...
    for package in state_node.package_history:
        ...
        answer = answer and spawn_time <= pick_up_time and delivery_time != -1 and final_time >= delivery_time > pick_up_time
    return answer
...
def heuristic_mst(state_node, distance_func=manhattan):
    current_position = state_node.current_position
    # graph_nodes = set(current_position | state_node.points_of_interest)
    graph_edges = []
    poi = list(state_node.points_of_interest)
    for i, s in enumerate(poi):
        distances, _ = distance_func(s, poi[i + 1:], state_node.grid)
        graph_edges+=[(s, t, distances[t]) for t in poi[i + 1:]]
        # for n1, n2 in combinations(graph_nodes, 2):
    #     graph_edges.append((n1, n2, abs(n1[0] - n2[0]) + abs(n1[1] - n2[1])))
    if len(graph_edges) == 0:
        return 0
    tree_weights = mst(poi, graph_edges)
    distances_from_current, _ = distance_func(current_position, poi, state_node.grid)
    connect_current_pos = min(distances_from_current[t] for t in poi)
